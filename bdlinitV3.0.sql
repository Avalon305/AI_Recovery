...
Source Server         : 远程水电平台服务器
Source Server Version : 100218
Source Host           : 49.4.69.9:3306
...
CREATE TABLE `bdl_deviceprescription` (
  ...
  `dp_memo` text DEFAULT NULL COMMENT '注意点、指示',
  ...
  `extra_setting` text DEFAULT NULL COMMENT '额外属性：为保证设备属性的可扩展性，存储为Json串Key、value',
...
CREATE TABLE `bdl_physicalpower` (
  ...
  `pp_usermemo` text DEFAULT NULL COMMENT '利用者感想',
  `pp_workermemo` text DEFAULT NULL COMMENT '工作人员感想',
...
CREATE TABLE `bdl_prescriptionresult` (
  ...
  `heart_rate_list` text DEFAULT NULL COMMENT '心率集合：运动过程实时心率集合，数据之间*分割',
  `gmt_create` timestamp NULL DEFAULT current_timestamp(),
  `gmt_modified` timestamp NULL DEFAULT NULL ON UPDATE current_timestamp(),
...
CREATE TABLE `bdl_set` (
  ...
  `set_unique_id` text DEFAULT NULL COMMENT '客户机唯一id',
...
CREATE TABLE `bdl_symptominfo` (
  ...
  `si_careinfo` text DEFAULT NULL COMMENT '看护记录',
...
CREATE TABLE `bdl_user` (
  ...
  `user_memo` text DEFAULT NULL COMMENT '备忘',
  ...
  `user_privateinfo` text DEFAULT NULL COMMENT '非公开信息',
