...
namespace NuitrackScanProgress.dao
{
    class SkeletonLengthDAO : BaseDAO<SkeletonLengthEntity>
    {
        ...
        public NuitrackEntity GetPk_user_idByStatus(int Status)
        {
            using (var conn = DbUtil.getConn())
            {
                const string query = "select * from bdl_nuitrack where status = @Status";

                return conn.QueryFirstOrDefault<NuitrackEntity>(query, new { Status });
